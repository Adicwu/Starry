<template>
	<div class="ad-carousel" ref="carousel" :style="{ height: height }" @mouseenter="clearMove" @mouseleave="autoMove" @touchstart.stop="touchStart" @touchend.stop="touchEnd">
		<div class="_contain" ref="contain" :class="{ 'is-animating': isTransition }" :style="{ transform: `translate3d(${moveX}px,0,0)` }"><slot></slot></div>
		<span class="_arrow _arrow-left" @click="debounce(prevItem)"><i class="fa fa-angle-left"></i></span>
		<span class="_arrow _arrow-right" @click="debounce(nextItem)"><i class="fa fa-angle-right"></i></span>
		<ul class="_indicators">
			<li v-for="item in items.length" :class="{ '_indicators-active': index == item - 1 }" :key="item"></li>
		</ul>
	</div>
</template>
...
export default {
	...
	computed: {
		moveX() {
			return this.items.childList ? -(this.index * this.items.childList[0].clientWidth) : 0;
		}
	},
	watch: {
		autoplay: {
	...
	methods: {
		...
		autoMove() {
			this.clearMove();
			this.autoplay && (this.timer = setInterval(() => this.nextItem(), this.interval));
		},
		clearMove() {
